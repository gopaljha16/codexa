...
const initializeSocket = (server) => {
  ...
  io.on('connection', (socket) => {
    ...
    // Join/leave coding challenge rooms
    socket.on('join-challenge', (roomId) => {
      if (!roomId) return;
      socket.join(`challenge:${roomId}`);
    });
    socket.on('leave-challenge', (roomId) => {
      if (!roomId) return;
      socket.leave(`challenge:${roomId}`);
    });
